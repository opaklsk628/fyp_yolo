...
def fall_records():
    ...
    try:
        # 取出 id 以便刪除
        cur.execute(
            "SELECT id, detection_time, lying_down_count, image_path "
            "FROM fall_records ORDER BY detection_time DESC"
        )
        for rid, dt, cnt, img in cur.fetchall():
            if img.startswith('static/'):
                img = img.split('/')[-1]
            if not img.startswith('log_photo/'):
                img = f"log_photo/{img}"
            records.append((rid, dt, cnt, img))
    except Exception as e:
...
@app.route('/fall_records/delete/<int:rid>', methods=['POST'])
@admin_required
def delete_fall_record(rid):
    """刪除單筆跌倒記錄（僅限管理員）。"""
    conn = get_database_connection()
    if not conn:
        flash('資料庫連線失敗')
    ...
    try:
        # 先抓圖片路徑，準備連同檔案一起刪
        cur.execute("SELECT image_path FROM fall_records WHERE id=%s", (rid,))
        row = cur.fetchone()
        if row:
            img_path = row[0]
        ...
        cur.execute("DELETE FROM fall_records WHERE id=%s", (rid,))
        conn.commit()
        flash('跌倒記錄已刪除')
    except Exception as e:
        conn.rollback()
        logger.error(f"刪除跌倒記錄失敗: {e}")
        flash('刪除失敗')
    finally:
        cur.close(); conn.close()
